defmodule JinjaTest do
  ...
  describe "dict loader" do
    ...
    test "template inheritance works" do
      ...
      {:ok, result} = Jinja.render_template("page", %{
        title: "Test Page",
        heading: "Welcome",
        body: "This is a test"
      })
    ...
    test "template with conditional blocks" do
      :ok = Jinja.load_template("conditional", """
      {% if show_greeting %}
      <h1>Hello {{ name }}!</h1>
      {% else %}
      <h1>Goodbye {{ name }}!</h1>
      ...
      {:ok, goodbye} = Jinja.render_template("conditional", %{show_greeting: false, name: "world"})
